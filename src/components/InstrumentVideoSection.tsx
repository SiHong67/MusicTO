    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded-md bg-indigo-50 dark:bg-indigo-950/80 border border-indigo-200 dark:border-indigo-500/30 flex items-center justify-center text-indigo-600 dark:text-indigo-400">
            <Video className="w-3.5 h-3.5" />
          </div>
          <span className="font-mono text-xs font-bold text-slate-800 dark:text-gray-200 uppercase tracking-wider">
            Instrument Tryout Videos
          </span>
          <span className="px-2 py-0.5 rounded-full bg-slate-200 dark:bg-white/10 text-indigo-700 dark:text-indigo-300 font-mono text-[11px] font-bold">
            {allVideos.length}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg border font-mono text-xs font-bold uppercase tracking-wider transition-all flex items-center gap-1.5 ${
              isAdding
                ? 'bg-slate-200 dark:bg-white/10 text-slate-800 dark:text-white border-slate-300 dark:border-white/20'
                : 'bg-indigo-50 dark:bg-indigo-950/80 text-indigo-700 dark:text-indigo-300 border-indigo-200 dark:border-indigo-500/40 hover:bg-indigo-100 dark:hover:bg-indigo-900/60 shadow-xs'
            }`}
          >
            {isAdding ? (
              <>
      ...
      {isAdding && (
        <div className="p-4 bg-slate-50 dark:bg-[#12121A] border border-indigo-200 dark:border-indigo-500/30 rounded-xl space-y-4 animate-fadeIn">
          {/* Instrument Selector */}
          <div>
            <label className="block text-[10px] font-mono text-slate-500 dark:text-gray-400 uppercase font-bold tracking-widest mb-1.5">
                ...
                return (
                  <button
                    ...
                    className={`p-2 rounded-lg border flex flex-col items-center gap-1 transition-all ${
                      isSel
                        ? 'bg-indigo-600 dark:bg-indigo-950/90 border-indigo-600 dark:border-indigo-500 text-white shadow-xs'
                        : 'bg-white dark:bg-[#181824] border-slate-200 dark:border-white/10 text-slate-600 dark:text-gray-400 hover:text-slate-900 dark:hover:text-white hover:border-slate-300 dark:hover:border-white/20'
                    }`}
                  >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-[10px] font-mono text-slate-500 dark:text-gray-400 uppercase font-bold tracking-widest mb-1">
              ...
              <input
                ...
                className="w-full bg-white dark:bg-[#181824] border border-slate-200 dark:border-white/10 rounded-lg px-3 py-2 text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-gray-500 focus:outline-none focus:border-indigo-500"
              />
            </div>
              ...
              <div
                ...
                className={`p-6 border-2 border-dashed rounded-xl flex flex-col items-center justify-center gap-2 cursor-pointer transition-colors text-center ${
                  isDragging
                    ...
                    : 'border-slate-300 dark:border-white/15 bg-white dark:bg-[#181824] text-slate-500 dark:text-gray-400 hover:border-indigo-500/50 hover:bg-slate-50 dark:hover:bg-white/5'
                }`}
              >
                {isProcessingFile ? (
                  <div className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400 font-mono text-xs">
                  ...
                  <>
                    ...
                    <p className="text-[10px] font-mono text-slate-400 dark:text-gray-500">
                ...
                <div className="flex items-center gap-1.5">
                  {sampleVideos.map((sample) => (
                    <button
                      ...
                      className="px-2 py-1 bg-slate-100 hover:bg-slate-200 dark:bg-white/5 dark:hover:bg-white/10 border border-slate-200 dark:border-white/10 rounded text-[10px] font-mono text-indigo-600 dark:text-indigo-300 flex items-center gap-1"
                    >
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-white dark:bg-[#181824] border border-slate-200 dark:border-white/10 rounded-lg px-3 py-2 text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-gray-500 focus:outline-none focus:border-indigo-500"
                  />
                  <button
                    type="submit"
                ...
                <p className="text-[10px] text-slate-500 dark:text-gray-500 font-mono mt-1">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-center gap-3">
                ...
                <button
                  type="button"
                  onClick={stopCamera}
                  className="px-3 py-2 text-xs text-slate-500 dark:text-gray-400 hover:text-slate-900 dark:hover:text-white font-mono"
                >
                  Close Camera
                </button>
              </div>
      ...
      {filteredVideos.length === 0 ? (
        <div className="p-4 bg-slate-50 dark:bg-[#12121A] border border-slate-200 dark:border-white/5 rounded-xl text-center">
          ...
          <button
            ...
            className="mt-2 text-xs text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300 font-mono font-medium underline"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
          {filteredVideos.map((vid) => {
            ...
            return (
              <div
                key={vid.id}
                className="group relative bg-white dark:bg-[#12121A] border border-slate-200 dark:border-white/10 hover:border-indigo-400 dark:hover:border-indigo-500/50 rounded-xl overflow-hidden shadow-xs hover:shadow-md transition-all flex flex-col justify-between"
              >
                {/* Video Card Thumbnail / Play trigger */}
                <div
                  onClick={() => onSelectVideo(vid)}
                  className="relative aspect-video bg-black/80 cursor-pointer overflow-hidden flex items-center justify-center group-hover:bg-black/60 transition-colors"
                >
                ...
                <div className="p-3 flex flex-col justify-between flex-1 gap-1">
                  <div>
                    <h4
                      onClick={() => onSelectVideo(vid)}
                      className="text-xs font-bold text-slate-900 dark:text-white hover:text-indigo-600 dark:hover:text-indigo-300 cursor-pointer line-clamp-1"
                    >
                      {vid.title || `${station?.name || vid.stationId} Tryout`}
                    </h4>
                    {vid.notes && (
                      <p className="text-[11px] text-slate-500 dark:text-gray-400 line-clamp-1 mt-0.5 italic">
                        "{vid.notes}"
                      </p>
                    )}
                  </div>

                  <div className="flex items-center justify-between text-[10px] font-mono text-slate-400 dark:text-gray-500 pt-2 border-t border-slate-100 dark:border-white/5">
